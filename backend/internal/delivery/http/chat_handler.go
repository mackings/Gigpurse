...
func (h *ChatHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	// Single realtime socket per user: carries live chat messages AND
	// notification pushes (see Hub / broadcastingNotificationRepository), so
	// the frontend only ever needs one persistent connection.
...
func (h *ChatHandler) HandleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if err := json.Unmarshal(messageBytes, &wsReq); err != nil {
			h.hub.Send(userID, "error", "invalid message format")
			continue
		}
		...
		// Send confirmation back to sender — routed through the hub (not a
		// direct conn.WriteJSON) so it shares the per-connection write lock
		// with any concurrent notification push landing on this same
		// connection (e.g. SendMessage above just created one for the
		// receiver, or this user separately received one at the same time).
		h.hub.Send(userID, "chat_message", msg)

		// Dispatch to receiver if online
		h.hub.Send(wsReq.RecvID, "chat_message", msg)
	}
}
