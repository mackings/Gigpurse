...
func main() {
	...
	notifRepo := delivery.NewBroadcastingNotificationRepository(mongodb.NewNotificationRepository(db), hub, delivery.NewPushSender(nil, "", "", ""))
	...
	jobUsecase := usecase.NewJobUsecase(jobRepo, userRepo, contractRepo, notifRepo, walletRepo, reviewRepo, paypetalClient, escrowAgreementRepo, milestoneRepo, "http://localhost:3000")
	...
	adminUsecase := usecase.NewAdminUsecase(db, userRepo, jobRepo, contractRepo, milestoneRepo, escrowAgreementRepo, jobUsecase, "http://localhost:3000")
	...
	contractHandler := delivery.NewContractHandler(contractUsecase, disputeUsecase)
	...
	milestoneHandler := delivery.NewMilestoneHandler(milestoneUsecase, disputeUsecase)
	...
	go func() {
		...
		for {
			...
			_ = json.Unmarshal(msgBytes, &envelope)
			if envelope.Type == "chat_message" {
				musicianCh <- envelope.Data
			}
		}
	}()
