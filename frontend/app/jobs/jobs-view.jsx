...
export default function JobsView() {
  ...
  // The open job panel lives in the URL (?job=<id>) — shareable, and the
  // browser back button naturally closes it, matching how Upwork's own job
  // detail panel behaves.
  const openJobId = searchParams.get("job");
  const openJob = (id) => router.push(`${pathname}?job=${id}`, { scroll: false });
  const closeJob = () => router.push(pathname, { scroll: false });
  ...
  const { data: openJobDetail, isLoading: openJobLoading } = useQuery({
    queryKey: ["job", openJobId],
    queryFn: () => apiGet(`/jobs?id=${openJobId}`),
    enabled: !!openJobId,
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Sheet open={!!openJobId} onOpenChange={(v) => !v && closeJob()}>
        <SheetContent>
          {openJobLoading || !openJobDetail ? (
            ...
            <>
              <SheetHeader className="sr-only">
                <SheetTitle>{openJobDetail.title}</SheetTitle>
                <SheetDescription>Gig details</SheetDescription>
              </SheetHeader>
              <JobDetailContent
                job={openJobDetail}
                currentUser={user}
                isAuthenticated={isAuthenticated}
                alreadyApplied={appliedJobIds.has(openJobDetail.id)}
                saved={savedJobIds.has(openJobDetail.id)}
                onApplied={onApplied}
                showOpenInNewWindow
              />
            </>
