...
const emptyAdvanced = { minGigs: "", minEarned: "", atRiskOnly: false };
...
export default function AdminTalent() {
  ...
  const { data: rows, isLoading } = useQuery({
    queryKey: ["admin-talent-engagement", window_],
    queryFn: () => apiGet(`/admin/talent-engagement?window=${window_}`),
  });

  const windowDays = Number(window_);
  const activeAdvancedCount =
    (advanced.minGigs ? 1 : 0) +
    (advanced.minEarned ? 1 : 0) +
  ...
  const filtered = useMemo(() => {
    ...
    return (rows || []).filter((r) => {
      ...
      if (advanced.minGigs && r.gigs_count < Number(advanced.minGigs))
        return false;
      if (advanced.minEarned && r.financial_total < Number(advanced.minEarned))
        return false;
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">
              Minimum total earned (₦)
            </label>
            <Input
              type="number"
              placeholder="0"
              value={advanced.minEarned}
              onChange={(e) =>
                setAdvanced({ ...advanced, minEarned: e.target.value })
              }
            />
          </div>
      ...
      {isLoading ? (
        ...
        <EngagementTable
          ...
          financialLabel="Total earned"
          emptyLabel={
            rows?.length
              ? "No talent match your filters."
              : "No talent on the platform yet."
          }
        />
      )}
    </div>
